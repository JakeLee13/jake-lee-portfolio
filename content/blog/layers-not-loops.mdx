---
title: "Layers, Not Loops: Building a Production Enterprise Churn Analysis System with LLMs"
date: "2025-02-25"
...
Every churned enterprise account has a story. But in the final call before churn, you'll often hear things like "It's just too expensive" or "We just weren't getting value to justify the spend." In turn, the CRM records get tagged as "Budget", or "Value Realization" -- in reality, the loss "reason" was spread over hundreds of calls with reps, thousands of emails, and millions of product interactions.

If you've worked in B2B SaaS, you know the ritual. A customer churns. The AE picks "Budget" from a dropdown. The CRM records it. Leadership reviews a pie chart of loss reasons next quarter and concludes that pricing is the problem. It's not that they're lying. It's that a single-select dropdown cannot capture what actually happened. Churn is almost always multi-causal, and it unfolds over months, sometimes years. The real story is buried across hundreds of calls with reps, thousands of emails, and millions of product interactions.

The loss field says "Budget" -- but the emails show a frustrated VP who stopped getting value six months ago, the call transcripts reveal a competitor evaluation that nobody flagged a year ago, and several of the product usage metrics show an inflection point around the same time the VP was frustrated. 

I built a system that triangulates all of this automatically: emails, call transcripts, and product usage metrics fed through an LLM pipeline that produces polished HTML reports -- per account, per quarter, and per year. No agents, nothing fancy. Just high quality input data, and deep context and prompt engineering, all in one clean notebook triggered on closed lost.

The result: Evidence-backed churn reports that reveal the actual causal chain behind each loss. Leaders can review the reports with the account team members, share them with executives, and use them to spot patterns across cohorts. And most importantly, for active accounts you can start to recognize the signals earlier and take action to drive better retention.

---

## The pipeline at a glance

Data flows from three sources -- Gong for call transcripts, People AI for emails, and Domo for product usage metrics -- through an ETL layer that stages everything, into a Jupyter notebook that runs the 4-stage LLM pipeline. Each stage has clearly defined inputs and outputs, and most stages run in parallel across accounts using our custom SDK.

<PipelineDiagram />

A quick note on what makes this work at the infrastructure level: boring data prep. Before any LLM touches anything, we've already done the ETL work, parsed out the email content we need, built up rich context cards for each account, and structured every input so the model can focus purely on analysis. We built a custom SDK (`domo_sdk`) that wraps LLM calls and Domo data I/O into a clean interface:


The SDK gives us super simple methods like `llm.prompt("Why did this account churn?)` for single LLM calls and `llm.parallel()` (see below) for fanning out across accounts with configurable worker counts and built-in retry logic. Here's what that looks like in practice:

<SdkComparison />

[edit] This is the foundation that makes rapid iteration possible. When youre able focus on iterating and improving your pipeline instead of gross llm syntax and debugging erros
...
Instead, we just z-score every metric against the account's own historical baseline and embed the z-score directly in the value string.

[edit: show simple z score formula]

[edit: help reader understand the interpretation of a z score and why comparing Card Loads vs datasets created would cause the model to possibly attribute larger magnitudes to more important. ALSO it helps with cross-account analysis later since all are on the same scale (a 400k account probably creates way more dataflows than a 40k account) ]The format `694(z=-1.8)` packs two pieces of information into a compact string: the raw value and its deviation from baseline. A z-score of -1.8 immediately tells the model "this is nearly two standard deviations below normal." No additional context needed.
...
This principle extends beyond LLM systems. It's the same reason MVC exists, the same reason you separate business logic from presentation in any well-architected system. The fact that it needs to be restated for LLM applications tells you how young this field is.
