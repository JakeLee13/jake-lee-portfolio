...
<SideNote label="Token">
The smallest unit the model reads. Roughly **¾ of an English word**, on average — "functionality" is one token, "dark" is one token, " dark" (with a leading space) is a different token. The model sees integer IDs, not characters. We tokenize at station 4.
</SideNote>

<SideNote label="System prompt">
A long block of instructions Anthropic pre-loads at the start of every Claude Code session — tone, tool-use rules, safety constraints, formatting conventions. Invisible to you, but counted against your token budget. Typically 5–8K tokens.
</SideNote>

<SideNote label="Tool schema">
A JSON description of one tool the model can call (its name, what arguments it takes, what it returns). Five or six tools' worth of schemas can run another 1–2K tokens. The model has to "see" these in every request to know what's callable.
</SideNote>
...
<SideNote label="TLS 1.3">
The encryption layer underneath HTTPS. **1.3** is the modern version: one round-trip handshake, modern ciphers (ChaCha20-Poly1305 / AES-GCM), forward-secret by default.
</SideNote>

<SideNote label="HTTP/2">
A streaming-capable version of HTTP. Lets the server flush partial responses (one token at a time) over the same connection. We use this at station 10 to stream tokens back as soon as they're generated.
</SideNote>

<SideNote label="Cloudflare PoP">
A point of presence — a datacenter where Cloudflare runs edge servers near you. Anthropic terminates TLS at Cloudflare's edge, then ships the decrypted request inside its own private network to the GPU pool.
</SideNote>

### 3. Routing — the cache decides the economics

<RequestPath station="03" />
...
<SideNote label="KV cache">
The model's working memory. For every past token in the conversation, the model stored two vectors (a "key" and a "value") inside attention. Those vectors are the conversation's residue on the GPU. Decode reads them every step. We size them at station 8.
</SideNote>

<SideNote label="Prefix cache hit">
When a new request's beginning (system prompt + tools + earlier turns) matches a request the same replica saw recently, that replica reuses the cached KV vectors instead of recomputing them. Claude Code's hit rate is **84–92%** — the difference between agents being routine and uneconomic.
</SideNote>

<SideNote label="Replica">
One copy of the model loaded onto a group of GPUs, ready to serve requests. A serving fleet runs many replicas in parallel; the router decides which one a request goes to.
</SideNote>
...
<SideNote label="BPE (byte-pair encoding)">
The training-time algorithm that built the tokenizer's vocabulary. Start from raw bytes. Find the most frequent adjacent byte pair across a giant text corpus, merge it into a new token, repeat ~50,000–200,000 times. The result is a vocabulary where common English words ("the", "and", " functionality") are single tokens, rare ones split into known subword chunks.
</SideNote>

<SideNote label="Subword tokenization">
Why not whole words? A real-world vocabulary has millions of entries (every name, typo, code identifier) and your embedding table would be enormous. Why not characters? Sequences become 5–10× longer and every cost scales with sequence length. Subwords are the compromise: roughly a 50K–200K vocab, common things in one piece, rare things in 2–4 pieces.
</SideNote>

<SideNote label="Vocabulary size">
For Llama-3 it's **128,256** tokens; for GPT-4 (o200k_base) it's **200,019**; for Gemma 2 it's **256,000**. Bigger vocabs cover more languages and code at fewer tokens per sentence, at the cost of a bigger embedding table.
</SideNote>
...
<SideNote label="Embedding">
A lookup table that turns each token ID into a fixed-length numeric vector. After this step, every token's identity is gone — what's left is its 8,192-dimensional **direction** in vector space. Two tokens with similar meaning have similar directions, learned during pretraining.
</SideNote>

<SideNote label="BF16">
"Brain float 16" — a 16-bit floating-point format. Half the bytes of FP32, same exponent range, less precision. The frontier inference default since 2023. **FP8** halves it again; **FP4** halves it once more — the Blackwell-era inference throughput win.
</SideNote>

<SideNote label="HBM">
High-Bandwidth Memory. Fast DRAM stacked vertically and soldered to the same silicon interposer as the GPU die. **H100 has 80 GB at 3.35 TB/s; H200 141 GB at 4.8 TB/s; B200 192 GB.** Capacity and bandwidth are different scarcities — we hit both at station 8.
</SideNote>

<SideNote label="lm_head">
The mirror-image table at the *end* of the stack. Same shape as the input embedding, but used in reverse: takes the final 8,192-vector and projects back to 128,256 scores — one per possible next token. We use it at station 9.
</SideNote>
...
<SideNote label="RoPE (rotary position embedding)">
Position information injected by rotating Q/K vectors as a function of token position. Low-frequency dimension-pairs spin fast (encode local order); high-frequency pairs barely move (encode coarse, long-range position). The trick is purely multiplicative — no learned parameters, no extra parameters.
</SideNote>

<SideNote label="Q / K / V">
Query, Key, Value. Three different linear projections of each token's embedding, used inside attention. **Q** asks "what am I looking for?", **K** says "what do I expose?", **V** says "what do I contribute if attended to?". RoPE rotates Q and K, not V.
</SideNote>

<SideNote label="Context-length extension">
Because RoPE encodes position in rotation frequencies, you can extend a model from 8K to 128K context by **rescaling the base frequency** and briefly fine-tuning. YaRN and Meta's llama3-scaling are the published methods. This is why context windows keep growing faster than model generations do.
</SideNote>
...
<SideNote label="Attention">
For each token, **look at every prior token, weighted by how relevant it is to me right now.** Compute scores via Q · Kᵀ, softmax, then take a weighted sum of V vectors. The output is the token's view of the conversation so far through this layer's "lens."
</SideNote>

<SideNote label="GQA (grouped-query attention)">
Llama-3 has 64 query heads but only **8** K/V heads — groups of 8 query heads share each K/V head. This cuts the KV cache size by 8×. Pure multi-head attention (the original 2017 design) would make Llama-3 70B undeployable: KV cache alone would exceed the weights.
</SideNote>

<SideNote label="MLP (multi-layer perceptron)">
A small two-layer feedforward network applied independently to each token. Llama-3 70B uses SwiGLU: gate × up → multiply → down, with intermediate dimension 28,672. **MLP weights are the majority of model parameters** — the place where most of the model's stored "knowledge" lives.
</SideNote>

<SideNote label="Residual stream">
Every layer adds its output to the layer's input rather than replacing it. The "residual stream" is the running accumulator that each token's vector represents — every layer's contribution is a refinement, not a replacement.
</SideNote>
...
<SideNote label="Prefill vs decode">
**Prefill** processes the entire prompt in parallel: one big matmul, compute-bound, expensive in FLOPs but bandwidth is fine. **Decode** generates one output token at a time: tiny matmul, memory-bandwidth-bound, FLOPs are nearly free but you still have to re-read every weight every step. Same GPU, two completely different bottlenecks.
</SideNote>
...
<SideNote label="KV cache size formula">
`2 × n_layers × n_kv_heads × head_dim × seq_len × batch × dtype_bytes`. The leading 2 is K plus V. `n_kv_heads` is the **post-GQA** count (8 for Llama-3 70B, not 64) — GQA is the reason this number is tractable.
</SideNote>

<SideNote label="PagedAttention">
The vLLM-era scheme: chunk the KV cache into fixed-size blocks (typically 16 tokens, ~80 KB each). Two requests that share a 50K prefix point at the same physical blocks. Copy-on-write only on divergence. **KV-memory waste drops from 60–80% to under 4%**. Native paged-KV support is now in FlashAttention-3 and FlashInfer.
</SideNote>
...
<SideNote label="Logits">
The raw real-valued scores the model emits before softmax. Logit 40 ("I") might be `8.2`; logit 1701 ("Looking") might be `7.1`. Bigger logit = more probable.
</SideNote>

<SideNote label="Softmax">
Turns a vector of arbitrary reals into probabilities that sum to 1. `exp(xᵢ) / Σⱼ exp(xⱼ)`. The exponential makes the biggest logit dominate; differences in logit space become ratios in probability space.
</SideNote>

<SideNote label="Temperature">
A divisor on logits before softmax: `softmax(logits / T)`. **T = 0** is deterministic — always pick the top logit. **T = 1** keeps the raw distribution. Higher T → more variety, more risk. Claude Code uses T ≈ 0 for code generation.
</SideNote>

<SideNote label="Top-p / top-k">
Sampling restrictions: **top-k** keeps the k highest-probability tokens before sampling; **top-p (nucleus)** keeps the smallest set whose cumulative probability exceeds p (e.g. 0.9). Both prune the long tail of garbage tokens.
</SideNote>
...
<SideNote label="SSE (Server-Sent Events)">
A one-way streaming protocol over HTTP. Server sends `event:` / `data:` lines; client reads them as a stream. Simpler than WebSockets, sufficient when only the server needs to push.
</SideNote>

### 11. Tool use — the loop that turns one request into twenty-two

<RequestPath station="11" />
...
<SideNote label="Stateless">
Each model forward pass is independent. The model doesn't "remember" anything between requests — the entire conversation history is part of the input on every turn. **The illusion of continuous reasoning is the loop, not the model.**
</SideNote>

<SideNote label="MCP / tool schemas">
Model Context Protocol: a JSON description of available tools, included in every request so the model knows what it can call. ~280–320 tokens per tool definition. **A rack of five MCP servers with ~50 tools consumes 30–60K tokens before you've typed anything.**
</SideNote>
...
<SideNote label="Tensor parallelism (TP)">
A way to spread one model across multiple GPUs by **slicing each weight matrix across the GPUs** and running pieces of the matmul in parallel. TP=8 means 8 GPUs share the work of every layer. Different from data parallelism (different requests on different GPUs) and pipeline parallelism (different layers on different GPUs).
</SideNote>

<SideNote label="All-reduce / all-gather">
Two of the standard collective communication primitives. **All-reduce** sums partial results across all GPUs and gives every GPU the total; **all-gather** concatenates per-GPU shards into the full tensor on every GPU. Both have to finish before the next layer can start — they sit on the critical path of every forward pass.
</SideNote>

<SideNote label="Tied embeddings">
A trick from Press & Wolf (2017): use the **same matrix** for both the input embedding (token ID → vector) and the output projection `lm_head` (vector → logits). Saves 2–8 GB per model. Llama-3 doesn't do this; Gemma 2 does, because its 256K vocab makes an untied `lm_head` disproportionately expensive.
</SideNote>
...
<SideNote label="YaRN">
"Yet another RoPE eNgineering" — the published state-of-the-art for stretching a model's context window. Combines NTK-aware frequency scaling with a temperature term that recalibrates attention as the context grows. The published cost: fine-tuning on **under 0.1% of original pretraining tokens** to recover quality at the longer context.
</SideNote>
...
<SideNote label="MLA (multi-head latent attention)">
DeepSeek's variant of attention: project K and V down to a **shared low-rank latent** (e.g. 512-dim) and store only that; reconstruct full K and V on the fly per attention step. ~4× smaller KV cache than GQA at matched quality, at the cost of extra compute per decode step.
</SideNote>

<SideNote label="MHA vs GQA vs MLA">
The three attention variants that matter today. **MHA** (multi-head): every query head has its own K/V — the 2017 default, now obsolete at scale. **GQA** (grouped-query): query heads share K/V in groups of 4–8 — the frontier default. **MLA**: K/V live in a low-rank shared latent — DeepSeek's invention, ~4× smaller cache.
</SideNote>

<SideNote label="FlashAttention">
A family of attention kernels (Dao et al., 2022 → FlashAttention-3, 2024) that compute attention without materializing the full N×N score matrix. Tiles the computation through SRAM. **2–10× faster** than naïve attention and required for long context to be tractable.
</SideNote>
...
<SideNote label="NVFP4">
NVIDIA's 4-bit floating-point format introduced with Blackwell. Half the bytes of FP8 with per-tensor scaling that handles outliers. The lever Blackwell-era throughput wins lean on most heavily — half the memory bandwidth per weight or per cached K/V doubles decode throughput on a bandwidth-bound workload.
</SideNote>
...
<SideNote label="Sliding-window attention">
Each attention layer only looks at the last W tokens, not the full history. KV cache cost stops growing past W. Information about earlier tokens still propagates upward through *stacked* layers — each layer carries forward a summary. Modern designs mix sliding and full-attention layers.
</SideNote>

<SideNote label="Attention sinks">
The empirical observation that the **first few tokens** of any sequence absorb a disproportionate share of attention regardless of content. Keeping those tokens in the KV cache (and evicting middle ones to save memory) preserves quality far better than naïvely keeping only the tail.
</SideNote>
...
<SideNote label="Mixture of Experts (MoE)">
Replace each transformer layer's single MLP with **N parallel experts** (each a smaller MLP) plus a tiny router network. The router scores all experts per token, picks top-K, and only those K run. **Total parameters scale with N; compute scales with K.** Cheaper per token at the cost of much bigger HBM footprint.
</SideNote>

<SideNote label="Active vs total parameters">
**Total** = all expert weights that have to be resident in HBM (the router might pick any of them on the next token). **Active** = only the K experts that actually compute for this token. The naming convention "A22B" = 22B active; "235B-A22B" means 235B total with 22B active.
</SideNote>
...
<SideNote label="Expert parallelism (EP)">
Spread the experts of an MoE across multiple GPUs. Each GPU holds a subset of experts. Every decode step requires shuffling each token's activations to the GPU that owns the expert the router chose for it (and back).
</SideNote>

<SideNote label="All-to-all">
A collective where every GPU sends a different chunk of data to every other GPU. The standard primitive for expert-parallel MoE serving. Its latency is the dominant cost of MoE decode — which is why staying inside one NVLink domain (NVL72) matters disproportionately.
</SideNote>

<SideNote label="NVL72 / NVLink domain">
NVIDIA's rack-scale topology: **72 GPUs** wired into a single NVLink switch fabric at ~1.8 TB/s per GPU. Inside that domain, all-to-all is ~50× faster than over InfiniBand between racks. NVL72 is why Blackwell-era MoE benchmarks blow out the previous generation — the model fits inside one copper domain.
</SideNote>

<SideNote label="InfiniBand">
The conventional inter-server fabric for HPC and AI clusters. Typically 400–800 Gbps per port today. Roughly **20× slower than NVLink** per GPU. Cross-rack collectives go over IB; intra-rack stay on NVLink.
</SideNote>
...
<SideNote label="Continuous batching">
Don't wait for a request to finish before adding the next one to the batch. Every decode step, evict finished requests and admit new ones into their slots. Pre-Orca systems re-batched at request boundaries and wasted most of the GPU on stragglers. **36.9× throughput win** at matched p50 latency over the previous default.
</SideNote>
...
<SideNote label="Speculative decoding">
A small "draft" model proposes k future tokens cheaply. The big target model verifies all k in one parallel forward pass (decode is bandwidth-bound, so verifying k tokens costs ~the same as decoding one). Accepted tokens are free; rejected ones fall back. **~2× speedup** at typical acceptance rates.
</SideNote>
...
<SideNote label="Disaggregated serving">
Run prefill and decode on **separate GPU pools**, ship the KV cache between them. Each pool can be sized for its own bottleneck. Connecting them requires high-bandwidth RDMA between pools — the concrete mechanism linking inference architecture to optical-networking demand.
</SideNote>

<SideNote label="TTFT / TPOT">
**TTFT** = time to first token, the latency before streaming starts (dominated by prefill). **TPOT** = time per output token, the streaming cadence the user feels (dominated by decode). Chat SLAs want both small. They pull in opposite directions on a colocated pool, which is why disaggregation pays off.
</SideNote>

<SideNote label="Goodput">
Throughput **conditioned on meeting both SLAs simultaneously**. A serving system that hits 100K tokens/sec at 10s TTFT has poor goodput; one that hits 60K tokens/sec at 200ms TTFT has higher goodput. The metric that matters when latency is part of the product.
</SideNote>
...
<SideNote label="FP4 / FP8 / BF16">
The dtype ladder for weights. **BF16** = 2 bytes/weight (Hopper-era default). **FP8** = 1 byte/weight (also Hopper, with quality calibration). **FP4** = 0.5 bytes/weight (Blackwell-only). Each halving doubles the maximum decode throughput on a bandwidth-bound workload — at the cost of more aggressive calibration to preserve quality.
</SideNote>

<SideNote label="MFU (model FLOPs utilization)">
What fraction of the GPU's peak FLOPs your forward pass actually uses. Prefill on a well-batched workload: ~50–70%. Decode at batch-1: under 1%. Decode at batch-64: ~28%. **MFU is the headline efficiency metric** — every serving technique in this section is a different way to raise it.
</SideNote>

<SideNote label="FLOPs / byte (arithmetic intensity)">
The ratio of compute to data movement for a given operation. The **roofline** on H100 is ~295 FLOPs/byte at BF16 — operations above that line are compute-bound, below are bandwidth-bound. Prefill sits well above. Decode at batch-1 sits well below. Batching shifts decode rightward toward the roofline.
</SideNote>
